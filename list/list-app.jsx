...
function Checkbox({ done, partner, onClick }) {
  return <button onClick={onClick} title={done ? 'Done' : 'Mark done'} aria-label={done ? 'Mark as not done' : 'Mark done'} style={{ width: 26, height: 26, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0, padding: 0, marginTop: 2, transition: 'all .15s', border: done ? ('2px solid ' + partner) : '2px solid #dcd4c6', background: done ? partner : '#fff' }}>{done && <CI.Check size={14} color="#fff" />}</button>;
}
...
function ItemCard({ item, partner }) {
  return (
    <div onClick={item.open} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: 16, background: '#fff', borderRadius: 18, cursor: 'pointer', border: item.important ? '1.5px solid #e7d3a3' : '1px solid transparent', boxShadow: '0 1px 2px rgba(58,53,47,.05),0 6px 16px rgba(58,53,47,.035)' }}>
      ...
      <button onClick={item.star} title="Mark important" aria-label={item.important ? 'Unmark important' : 'Mark important'} style={iconBtn}><CI.Star size={19} filled={item.important} /></button>
    </div>
  );
}
...
function LabelsPanel({ v, partner, maxWidth }) {
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 18, padding: 16, boxShadow: '0 1px 2px rgba(58,53,47,.05),0 8px 22px rgba(58,53,47,.05)', display: 'flex', flexDirection: 'column', gap: 14, maxWidth }}>
      ...
      {v.customLabels.length ? <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>{v.customLabels.map(cl => (<div key={cl.id} style={{ display: 'flex', alignItems: 'center', gap: 10 }}><span style={cl.dotStyle} /><input value={cl.name} onChange={cl.rename} style={labelInput} /><button onClick={cl.remove} aria-label="Delete label" style={{ background: '#f6ece9', border: 'none', color: '#b07a6e', width: 40, height: 40, borderRadius: 10, fontSize: 16, cursor: 'pointer', flexShrink: 0, lineHeight: 1 }}>×</button></div>))}</div> : <span style={{ fontSize: 13, color: '#b3a99c', fontWeight: 600 }}>No labels yet — add one below.</span>}
...
function AddModal({ v, primary }) {
  ...
  return (
    <Overlay onClose={() => v.a.set({ addOpen: false })}><Sheet stop={v.stop} maxWidth={380}>
      <div style={{ padding: '22px 22px 14px', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}><h2 style={modalTitle}>Add</h2><button onClick={() => v.a.set({ addOpen: false })} aria-label="Close" style={closeX}>×</button></div>
    ...
    <Overlay onClose={() => v.a.set({ detailId: null, editing: false })} z={1000}><Sheet stop={v.stop} maxWidth={380} style={{ maxHeight: '92vh' }}>
      <div className="tog-scroll" style={{ overflowY: 'auto', padding: '22px 22px 8px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}><button onClick={v.a.duplicateItem} title="Duplicate" aria-label="Duplicate" style={{ border: 'none', background: '#ece6db', color: '#7a7166', width: 40, height: 40, borderRadius: 9, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="9" y="9" width="11" height="11" rx="2"/><path d="M5 15V5a2 2 0 0 1 2-2h10"/></svg></button><button onClick={() => v.a.toggleImportant(it.id)} title="Mark important" aria-label={it.important ? 'Unmark important' : 'Mark important'} style={{ border: 'none', background: 'none', padding: 9, cursor: 'pointer', lineHeight: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><CI.Star size={22} filled={it.important} /></button><button onClick={() => v.a.set({ detailId: null, editing: false })} aria-label="Close" style={closeX}>×</button></div>
        </div>
        {!editing ? (
          <Fragment>
...
function Board({ v, isDesktop, primary, partner, listName, iconImg }) {
  ...
  function ExportSheet({ onClose }) {
    ...
    return (<Overlay onClose={onClose} z={1300}><Sheet stop={v.stop} maxWidth={380}>
      <div style={{ padding: '22px 22px 14px', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}><div><h2 style={modalTitle}>Export list</h2><p style={{ margin: '4px 0 0', fontSize: 14, color: '#9a9186', fontWeight: 600 }}>Choose what to copy.</p></div><button onClick={onClose} aria-label="Close" style={closeX}>×</button></div>
...
function BoardShell({ sx }) {
  ...
  return (
    <div className="app">
      ...
      <button onClick={() => v.a.set({ bugOpen: true, bugSent: false })} title="Report a problem" aria-label="Report a problem" style={{ position: 'fixed', top: 24, right: 24, zIndex: 900, width: 46, height: 46, borderRadius: '50%', border: '1px solid #ecd9c4', background: '#fffaf3', color: '#b07d42', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', boxShadow: '0 2px 4px rgba(58,53,47,.08),0 8px 20px rgba(58,53,47,.12)' }}><CI.Bug size={21} /></button>
