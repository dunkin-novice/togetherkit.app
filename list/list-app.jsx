  ...
  return (
    <div style={{ padding: isDesktop ? '38px 44px 46px' : '28px 18px 40px', display: 'flex', flexDirection: 'column', gap: isDesktop ? 22 : 18 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {v.items.map(item => (<CSwipeRow key={item.id} onDelete={item.deleteSelf} onComplete={item.doneSelf} completeLabel={item.done ? 'Undo' : 'Done'}><ItemCard item={item} partner={partner} /></CSwipeRow>))}
      </div>
