...
const rowToW = (r) => ({ id: r.id, exercise: r.exercise, weight: r.weight, unit: r.unit || 'kg', reps: r.reps, date: r.log_date, byUser: r.by_user, byName: r.by_name });
...
function useFitnessStore(homespaceId, me) {
  ...
  const [state, setState] = useState(() => ({
    ...
    wAddOpen: false, wDraft: { exercise: '', weight: '', unit: 'kg', reps: '', date: today() }, exFocus: false,
  ...
  const actions = useMemo(() => ({
    set: patch,
    setWDraft: (p) => patch(s => ({ wDraft: { ...s.wDraft, ...p } })),
    setBDraft: (p) => patch(s => ({ bDraft: { ...s.bDraft, ...p } })),
    addWorkout: () => {
      const s = ref.current, d = s.wDraft, m = meRef.current || { uid: null, name: 'Me' };
      const exercise = (d.exercise || '').trim(); if (!exercise) return;
      const w = { id: BE.newId(), exercise, weight: d.weight === '' ? null : Number(d.weight), unit: d.unit, reps: d.reps === '' ? null : Number(d.reps), date: d.date, byUser: m.uid, byName: m.name };
      patch(st => ({ workouts: [...st.workouts, w].sort((a, b) => (a.date < b.date ? -1 : 1)), wAddOpen: false, exFocus: false, graphExercise: st.graphExercise || exercise, wDraft: { exercise: '', weight: '', unit: d.unit, reps: '', date: today() } }));
      db(client.from('fitness_logs').insert({ id: w.id, homespace_id: homespaceId, exercise, weight: w.weight, unit: w.unit, reps: w.reps, log_date: w.date, by_user: w.byUser, by_name: w.byName, pos: Date.now() }));
    },
    addBody: () => {
...
function Overlay({ onClose, z = 1200, children }) { return <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: z, background: 'rgba(58,53,47,.42)', backdropFilter: 'blur(3px)', WebkitBackdropFilter: 'blur(3px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 18 }}>{children}</div>; }
function Sheet({ stop, maxWidth, children }) { return <div onClick={stop} style={{ width: '100%', maxWidth, background: '#f6f4ef', borderRadius: 26, boxShadow: '0 24px 60px rgba(58,53,47,.34)', overflow: 'hidden', maxHeight: '92vh', display: 'flex', flexDirection: 'column' }}>{children}</div>; }
...
function buildView(state, actions, opts) {
  ...
  const wSeriesMap = {};
  state.workouts.filter(w => w.exercise === ge && w.weight != null).forEach(w => {
    const k = w.byUser || 'x'; wSeriesMap[k] = wSeriesMap[k] || {};
    const day = w.date; const v = Number(w.weight);
...
function AddWorkout({ v, primary }) {
  ...
  const d = v.s.wDraft;
  const suggestions = v.exSuggestions(d.exercise);
  return (
    <Overlay onClose={() => v.a.set({ wAddOpen: false, exFocus: false })}>
      <Sheet stop={v.stop} maxWidth={380}>
        <div style={{ padding: '22px 22px 12px', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}><h2 style={modalTitle}>Log a set</h2><button onClick={() => v.a.set({ wAddOpen: false, exFocus: false })} style={closeX}>×</button></div>
        <div className="tog-scroll" style={{ overflowY: 'auto', padding: '0 22px', display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{ position: 'relative' }}>
            <input value={d.exercise} onChange={(e) => v.a.setWDraft({ exercise: e.target.value })} onFocus={() => v.a.set({ exFocus: true })} placeholder="Search exercise — e.g. Bench Press" autoFocus style={{ ...fieldInput, fontFamily: "'Quicksand',sans-serif", fontSize: 16 }} />
            {v.s.exFocus && suggestions.length > 0 && (
              <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, marginTop: 5, background: '#fff', border: '1px solid #ece6db', borderRadius: 13, boxShadow: '0 10px 26px rgba(58,53,47,.14)', zIndex: 5, maxHeight: 220, overflowY: 'auto', padding: 5 }}>
                {suggestions.map(name => <button key={name} onMouseDown={(e) => { e.preventDefault(); v.a.set({ wDraft: { ...v.s.wDraft, exercise: name }, exFocus: false }); }} style={{ display: 'block', width: '100%', textAlign: 'left', border: 'none', background: 'none', padding: '10px 11px', borderRadius: 9, fontSize: 14.5, fontWeight: 700, color: '#3a352f', cursor: 'pointer', fontFamily: 'inherit' }}>{name}</button>)}
              </div>
            )}
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <input value={d.weight} onChange={(e) => v.a.setWDraft({ weight: e.target.value })} type="number" inputMode="decimal" placeholder="Weight" style={{ ...fieldInput, flex: 1 }} />
            <div style={{ display: 'flex', borderRadius: 12, overflow: 'hidden', border: '1px solid #ece6db' }}>{UNITS.map(u => <button key={u} onClick={() => v.a.setWDraft({ unit: u })} style={{ border: 'none', padding: '0 14px', fontWeight: 800, fontSize: 14, cursor: 'pointer', fontFamily: 'inherit', background: d.unit === u ? primary : '#fff', color: d.unit === u ? '#fff' : '#9a9186' }}>{u}</button>)}</div>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <input value={d.reps} onChange={(e) => v.a.setWDraft({ reps: e.target.value })} type="number" inputMode="numeric" placeholder="Reps (optional)" style={{ ...fieldInput, flex: 1 }} />
            <input value={d.date} onChange={(e) => v.a.setWDraft({ date: e.target.value })} type="date" style={{ ...fieldInput, flex: 1 }} />
          </div>
        </div>
        <div style={{ padding: '16px 22px 22px', display: 'flex', gap: 10 }}><button onClick={() => v.a.set({ wAddOpen: false, exFocus: false })} style={cancelBtn}>Cancel</button><button onClick={v.a.addWorkout} style={{ flex: 2, background: primary, color: '#fff', border: 'none', borderRadius: 14, padding: 13, fontWeight: 800, fontSize: 14.5, cursor: 'pointer', fontFamily: 'inherit' }}>Save set</button></div>
      </Sheet>
    </Overlay>
  );
}
      ...
      {tab === 'compare' ? <CompareSection v={v} /> : tab === 'workouts' ? (
        <Fragment>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
            ...
            {v.workouts.map(w => (
              <div key={w.id} style={{ ...card, padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 15, fontWeight: 800, color: '#3a352f', fontFamily: "'Quicksand',sans-serif" }}>{w.exercise}</div>
                  <div style={{ fontSize: 13, fontWeight: 700, color: '#7a7166', marginTop: 2 }}>{w.weight != null ? w.weight + ' ' + w.unit : 'bodyweight'}{w.reps ? ' × ' + w.reps : ''}</div>
                </div>
                ...
                <button onClick={w.remove} title="Delete" style={{ border: 'none', background: 'none', color: '#cbb9a2', cursor: 'pointer', padding: 4, lineHeight: 0 }}><FI.Trash size={16} /></button>
              </div>
            ))}
...
function BoardShell({ sx }) {
  ...
  return (
    <div className="app">
      ...
      <AddWorkout v={v} primary={primary} /><AddBody v={v} primary={primary} />
